...
import { Pair, Bundle, Token, KoffeeSwapFactory, KoffeeSwapDayData, PairDayData, TokenDayData } from '../types/schema'
import { ONE_BI, ZERO_BD, ZERO_BI, FACTORY_ADDRESS } from './helpers'

export function updateKoffeeSwapDayData(event: EthereumEvent): KoffeeSwapDayData {
  let koffeeSwap = KoffeeSwapFactory.load(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400
  let dayStartTimestamp = dayID * 86400
  let koffeeSwapDayData = KoffeeSwapDayData.load(dayID.toString())
  if (koffeeSwapDayData === null) {
  ...
  return koffeeSwapDayData as KoffeeSwapDayData
}
